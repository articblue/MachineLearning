__author__ = 'Articblue: https://kaggle.com/articblue'
...
def get_im_cv2(path):
    ...
    resized = cv2.resize(img, (input_shapex,input_shapey),interpolation= cv2.INTER_LINEAR)
    return resized
...
def load_test():
    path = os.path.join('./', 'test', 'test_stg1', '*.jpg')
...
def create_model():
    ...
    model.add(Flatten())
    model.add(Dense(input_shapex, activation='relu'))
    model.add(Dropout(0.5))
    model.add(Dense(input_shapey, activation='relu'))
    model.add(Dropout(0.5))
...
def save_models(models):
    ...
    for i in range(nfolds):
        model = models[i]
        model.save(model_name+str(i)+model_ext)
